...
if active_model is not None:
    ...
    # --- XGBOOST STRICT COLUMN ORDER FIX ---
    expected_cols = active_model.get_booster().feature_names
    
    # Auto-Repair: Dynamically inject any missing lingering columns (like 'Power_Delta') with a neutral 0.0 value
    for col in expected_cols:
        if col not in input_df.columns:
            input_df[col] = 0.0
            
    try:
        # Reorder our dataframe to match the model's exact memory
        input_df = input_df[expected_cols]
    except KeyError as e:
        # Fallback error catch
        st.error(f"**Data Mismatch:** The model expects different feature names than what the app provided.")
        st.warning(f"Expected features: {expected_cols}")
        st.stop()
